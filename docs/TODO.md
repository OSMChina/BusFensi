...
1. [ ] 新建公交线路。需要的操作为：
    ...
    - [X] 手动按顺序选择一定的路径，得到公交线路。这里可能需要用到一些绘制的操作，即通过鼠标指定控制点等方式灵活选择。可能需要处理：双向公交。
    - [ ] 自动按照站点生成公交线路。暂时还没想好怎么实现
    - [X] 对线路元信息的修改，包括名字，运营单位等等

2. [X] 编辑公交线路。即对于已经存在的公交进行编辑

    - [X] 对站点进行新增，删除，重新排序
    - [X] 对路径进行拓展，收缩，删除和重新连接。
    - [X] 对线路元信息的修改，包括名字，运营单位等等

3. [X] 删除公交线路
    
    - [X] 直接删除即可，后续可能会考虑拼接线路的需求
...
- [X] <s>**(MEDIUM)** computed 属性性能较差，需要优化。</s>目前去掉了 computed middleware 中 collection 的逻辑，实测性能过关。
...
- [ ] **(MEDIUM)** 没有定义多个公交站反复加入的逻辑。可以用 shift + right click